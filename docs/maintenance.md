...
7. **Timer Drift and Synchronization:** For UI clocks or timers that update infrequently (e.g., every minute), avoid using `setInterval` with short intervals (like 1000ms). Instead, calculate the exact milliseconds until the next update and use a recursive `setTimeout`. Combine this with a `visibilitychange` event listener to force a resync when the tab is re-focused, preventing clock drift caused by browser throttling and reducing unnecessary main thread wakeups.
