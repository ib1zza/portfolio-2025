...
interface WindowManagerStore {
  ...
  windowHistory: Record<string, Pick<WindowInstance, "position" | "size">>;
...
const areSizesEqual = (a: WindowInstance["size"], b: WindowInstance["size"]) =>
    ...
    (set, get) => ({
      ...
      moveWindow: (id, position) =>
        set((state) => {
          ...
          if (
            !currentWindow ||
            arePositionsEqual(currentWindow.position, position)
          ) {
            return state;
          }
      ...
      closeWindow: (id) =>
        set((state) => {
          ...
          const openerWindowId =
            ...
            newWindows[closingWindow.openerWindowId]
              ? closingWindow.openerWindowId
              : undefined;
      ...
      closeFocusedWindow: () =>
        set((state) => {
          ...
          if (closingWindow?.fileId)
            delete newOpenFileIds[closingWindow.fileId];
          const openerWindowId =
            closingWindow?.openerWindowId &&
            newWindows[closingWindow.openerWindowId]
              ? closingWindow.openerWindowId
